...
export const AnimatedTestimonials = ({
  ...
  const safeTestimonials = Array.isArray(testimonials)
    ? testimonials.filter(t => t && t.name && t.quote)
    : [];
  if (!safeTestimonials.length) {
    return <div className="text-center text-muted-foreground py-12">No testimonials to display.</div>;
  }
        ...
        <div className="flex justify-between flex-col py-4">
          ...
          >
            <h3 className="text-2xl font-bold text-foreground">
              {safeTestimonials[active].name}
            </h3>
            <p className="text-sm text-muted-foreground">
              {safeTestimonials[active].designation}
            </p>
            <motion.p className="text-lg text-muted-foreground mt-8">
              {safeTestimonials[active].quote.split(' ').map((word, index) => (
                <motion.span
                  key={index}
                  initial={{
                    filter: 'blur(10px)',
