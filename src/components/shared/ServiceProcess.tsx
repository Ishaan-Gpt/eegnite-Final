    ...
    useEffect(() => {
        const calculateRange = () => {
            if (containerRef.current) {
                ...
                if (lastCard) {
                    const targetOffset = lastCard.offsetLeft + (lastCard.offsetWidth / 2) - (windowWidth / 2);
                    setScrollRange(Math.max(0, targetOffset));
                } else {
                    setScrollRange(Math.max(0, containerRef.current.scrollWidth - windowWidth));
                }
            }
        };
            ...
            <div className="sticky top-0 flex h-screen items-center overflow-hidden">
                ...
                <div className="absolute top-16 md:top-24 left-0 w-full z-20 text-left md:text-center">
                    <div className="max-w-[1400px] mx-auto px-6">
                        <h2 className={cn("text-3xl md:text-5xl lg:text-6xl font-bold uppercase tracking-tight text-black leading-tight text-left md:text-center", titleClassName)}>
                            {title}
                        </h2>
                    </div>
                </div>
                ...
                <div className="absolute bottom-8 md:bottom-12 left-0 w-full z-20 flex justify-center">
                    <a href={ctaLink || "tel:+916289753474"} className="bg-black text-white px-8 py-4 md:px-10 md:py-5 rounded-full font-bold uppercase tracking-widest text-xs md:text-sm hover:bg-[#FF6105] transition-colors shadow-xl">
