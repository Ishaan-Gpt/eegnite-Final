...
export function IndustryWhyUs({ badgeText, title, reasons, titleClassName }: IndustryWhyUsProps) {
        ...
        <>
            <section className="block md:hidden bg-white text-black py-16 px-6">
                <div className="max-w-[1400px] mx-auto">
                    {badgeText && (
                        <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-[#FF6105] mb-4 leading-normal text-balance">
                            {badgeText}
                        </p>
                    )}
                    <h2 className={cn("text-2xl font-bold uppercase tracking-tighter leading-[1.1] mb-10 text-black", titleClassName)}>
                        {title}
                    </h2>
                ...
                <div className="sticky top-0 flex flex-col justify-center h-screen overflow-hidden">
                    <div className="absolute top-32 left-0 w-full z-20">
                        <div className="max-w-[1400px] mx-auto px-6">
                            {badgeText && (
                                <p className="text-xs font-bold uppercase tracking-wider text-[#FF6105] mb-3 leading-normal text-center w-full mx-auto text-balance">
                                    {badgeText}
                                </p>
                            )}
                            <h2 className={cn("text-2xl md:text-4xl lg:text-[2.75rem] font-bold uppercase tracking-tighter leading-none text-center", titleClassName)}>
                                {title}
                            </h2>
                        </div>
                    </div>
                    <div className="flex items-center h-full w-full pt-48">
